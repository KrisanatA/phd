...
```{r}
nodes_sf <- st_as_sf(nodes, coords = c("latitude", "longitude"), crs = st_crs(4326))
...
# data frame way
x <- sf::st_coordinates(caribou_path)[, "X"]
y <- sf::st_coordinates(caribou_path)[, "Y"]
...
```{r}
ggraph(caribou_path, layout = layout_sf) +
...
There is a problem with the ggraph where its coordinates do not match with the sf. I will need to figure this out.

Next step:

1. Find why the ggraph and map do not match
2. Need to try linestring geometry. Maybe that is the problem
